package com.xqxy.dr.modular.event.mapper;

import com.baomidou.mybatisplus.core.conditions.Wrapper;
import com.baomidou.mybatisplus.core.mapper.BaseMapper;
import com.baomidou.mybatisplus.core.toolkit.Constants;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.xqxy.dr.modular.event.entity.CustExecute;
import com.xqxy.dr.modular.event.entity.CustExecute;
import com.xqxy.dr.modular.event.entity.EventExecute;
import com.xqxy.dr.modular.event.param.CustExecuteParam;
import org.apache.ibatis.annotations.Param;
import org.apache.ibatis.annotations.Select;

import java.util.List;

/**
 * <p>
 * 电力用户执行信息 Mapper 接口
 * </p>
 *
 * @author Shen
 * @since 2021-10-27
 */
public interface CustExecuteMapper extends BaseMapper<CustExecute> {

    /**
     * 事件实时监测
     * @param eventId
     * @param pageNo
     * @param pageSize
     * @return
     * @author shen
     * @date 2021-07-06
     */
    @Select("<script>" +
            "SELECT\n" +
            "\texecute_time executeTime,\n" +
            "\tSUM( execute_cap ) executeCap,\n" +
            "\tSUM(baseline_cap) baselineCap,\n" +
            "\tSUM( max_load_baseline ) maxLoadBaseline,\n" +
            "\tSUM( reply_cap ) replyCap,\n" +
            "\tSUM(max_execute_cap) maxExecuteCap\n" +
            "FROM\n" +
            "\tdr_cust_execute \n" +
            "WHERE\n" +
            "\tevent_id = #{eventId} \n" +
            "GROUP BY\n" +
            "\texecute_time\n" +
            "ORDER BY execute_time DESC" +
            "<if test=\"pageSize != 0 \">\n" +
            "   LIMIT #{pageNo}, #{pageSize}\n" +
            "</if>" +
            "</script>")
    List<CustExecute> eventMonitoring(Long eventId,int pageNo,int pageSize);

    Page<CustExecute> selectPageVo(Page<Object> defaultPage, @Param(Constants.WRAPPER) Wrapper ew);

    CustExecute getCustBaseByEventId(CustExecuteParam custExecuteParam);
}
