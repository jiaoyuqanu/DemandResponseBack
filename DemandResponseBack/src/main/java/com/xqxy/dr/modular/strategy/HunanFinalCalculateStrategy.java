    ...
    public Runnable createFirstInvitationDetail(Long eventId, LocalDateTime deadlineTime,String replySource,Integer regulateMultiple,Integer endCondition) {
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                int custCount = custInvitationService.count(custInvitationLambdaQueryWrapper);
                ...
                try {
                    ...
                    if(consCount==0) {
                        ...
                        LambdaQueryWrapper<ConsInvitation> consInvitationLambdaQueryWrapper2 = new LambdaQueryWrapper<>();
                        consInvitationLambdaQueryWrapper2.eq(ConsInvitation::getEventId,eventId);
                        List<ConsInvitation> consInvitationList = consInvitationService.list(consInvitationLambdaQueryWrapper2);
                        String sql = "update dr_cons_invitation set deadline_time=?,update_time=? where invitation_id = ?";
                        //获取数据库连接
                        conn = DriverManager.getConnection(url, user, password);
                        conn.setAutoCommit(false);
                        preparedStatement = conn.prepareStatement(sql);
                        ...
                        for (PlanCons planCon : planCons) {
                            ...
                            List<ConsInvitation> consInvitations = consInvitationList.stream().filter(consInvitation -> planCon.getConsId().equals(consInvitation.getConsId())
                                    && consInvitation.getEventId().equals(eventId)
                            ).collect(Collectors.toList());
                            if(null!=consInvitations && consInvitations.size()>0) {
                                preparedStatement.setLong(3, consInvitations.get(0).getInvitationId());
                            } else {
                                continue;
                            }
                            preparedStatement.addBatch();
                    ...
                    if(custCount==0) {
                        ...
                        LambdaQueryWrapper<CustInvitation> custInvitationLambdaQueryWrapper2 = new LambdaQueryWrapper<>();
                        custInvitationLambdaQueryWrapper2.eq(CustInvitation::getEventId,eventId);
                        List<CustInvitation> custInvitationList = custInvitationService.list(custInvitationLambdaQueryWrapper2);
                        String sql2 = "update dr_cust_invitation set deadline_time=?,update_time=? where invitation_id=?";
                        int j = 0;
                        PlanCustParam planCustParam = new PlanCustParam();
                        planCustParam.setPlanId(planConsParam.getPlanId());
                        List<PlanCust> planCusts = planCustService.listNotDeleted(planCustParam);
                        ...
                        for (PlanCust planCust : planCusts) {
                            ...
                            List<CustInvitation> custInvitations = custInvitationList.stream().filter(custInvitation -> planCust.getCustId().equals(custInvitation.getCustId())
                                    && custInvitation.getEventId().equals(eventId)
                            ).collect(Collectors.toList());
                            if(null!=custInvitations && custInvitations.size()>0) {
                                preparedStatement2.setLong(3, custInvitations.get(0).getInvitationId());
                            } else {
                                continue;
                            }
                            preparedStatement2.addBatch();
