...
public interface EventMapper extends BaseMapper<Event> {
    ...
    @Select("select * from dr_event where regulate_id = #{regulateId}")
    List<Event> selectEventByRegulateId(Long regulateId);

    @Update("update dr_event set " +
            "REGULATE_CAP = #{regulateCap},RANGE_TYPE = #{rangeType}," +
            "REGULATE_RANGE = #{regulateRange},END_TIME = #{endTime}," +
            "UPDATE_TIME = now()" +
            "where regulate_id = #{regulateId}")
    int updateRegulateEvent(Event event);
}
