...
public class ConsController {
     ...
     * @return
     */
    @ApiOperation(value = "根据custId 导出户号详情", notes = "根据custId 导出户号详情", produces = "application/json")
    @PostMapping("/exportConsByCustId")
    public ResponseData exportConsByCustId(@RequestBody CustParam custParam, HttpServletResponse response, HttpServletRequest request) {
        List<Cons> consList = new ArrayList<>();
        List<ExportCons> exportConsList = new ArrayList<>();
        if(StringUtils.isEmpty(custParam.getCustId())){
            consList = consService.getAllCons(custParam);
        }else {
            consList = consService.getConsListByCustId(custParam);
        }
        for (Cons cons : consList) {
            ExportCons exportCon = new ExportCons();
            BeanUtils.copyProperties(cons,exportCon);
            exportConsList.add(exportCon);
        }

        ExportParams params;
        HashMap<String,Object> map = new HashMap<>();
        map.put(NormalExcelConstants.DATA_LIST, exportConsList);
        map.put(NormalExcelConstants.CLASS, ExportCons.class);
