 ...
 */
@Component
public class BaseLineAndPlanJob {
     ...
     * @throws Exception
     */
    @XxlJob("synchroInvitationToPlan")
    public ReturnT<String> synchroInvitationToPlans(String param) throws Exception {
        LocalDateTime deadlineTime = LocalDateTime.now();
        String time = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")
                .format(deadlineTime);
        XxlJobLogger.log("同步用户邀约信息到方案表:{}" ,time);
        this.synchroInvitationToPlan(param);
        // default success
        return ReturnT.SUCCESS;
    }
     ...
     * @throws Exception
     */
    @XxlJob("synchroCustInvitationToPlan")
    public ReturnT<String> synchroCustInvitationToPlans(String param) throws Exception {
        XxlJobLogger.log("同步客户邀约信息到方案表");
        this.synchroCustInvitationToPlan(param);
        // default success
        return ReturnT.SUCCESS;
    }
    ...
    public void createCustBaseline(String param) {
        //获取客户基线
        List<CustBaseLineDetail> list = baseLineService.getCustBaseLine();
    ...
    public Runnable createCustBaseline(List<CustBaseLineDetail> list) {
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                List<CustBaseLineDetail> cureList = baseLineService.getCustCurve(sampleDateList,conIds);
                ...
                if(null!=list && list.size()>0) {
                    ...
                    lambdaQueryWrapper.eq(CustBaseLineDetail::getBaselineLibId,baselinId);
                                ...
                                try {
                                    for (int i = startIndex; i <= endIndex; i++) {
                                        ...
                                        for (CustBaseLineDetail consCurve : custBaseLineDetails) {
                                            ...
                                            //去除0值和null值数据
                                            if (null != power) {
                                                total = NumberUtil.add(total,power);
                    ...
                    String sql = "INSERT INTO dr_cust_baseline (\n" +
                    ...
                    try {
                        ...
                        String sql3="UPDATE dr_cust_baseline  \n" +
                        ...
                        if(null!=baseLineDetailList && baseLineDetailList.size()>0) {
                            int j = 0;
                            String sql2 ="UPDATE dr_cons_baseline set state = ? WHERE baseline_id = ?";
                        ...
                        if(null!=baseLine.getCustNum()) {
                            baseLine.setCustNum(baseLine.getCustNum()+1);
                            baseLineService.updateById(baseLine);
                        } else {
                            baseLine.setCustNum(1);
                            baseLineService.updateById(baseLine);
                        }
                    }
                }
    ...
    public void createBaseline(String param) {
        //一次只查询一个基线库任务
        List<PlanBaseLine> planBaseLines = planBaseLineService.getPlanBaseLineList();
    ...
    public Runnable createBaseline(List<PlanBaseLine> planBaseLines) {
        ...
        try{
            for (int j=1; j<=96; j++){
                ...
                consMethodMap4.put(j, EventPowerSample.class.getMethod("setP"+j, BigDecimal.class));
                consMethodMap5.put(j, EventPowerSample.class.getMethod("getP"+j));
            }
        } catch (Exception e) {
            e.getMessage();
        }
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                if(null!=planBaseLines && planBaseLines.size()>0) {
                    ...
                    try {
                        //开始下标
                        int startIndex = CurveUtil.covDateTimeToPoint(startTime);
                        int endIndex = CurveUtil.covDateTimeToPoint(endTime);
                        //查询已有用户基线集合
                        List<ConsBaseline> consBaselines = baseLineService.getConsBaseLineByBaseLineId(baseLine.getBaselinId());
                        //查询样本日期内的历史曲线集合
                        List<String> sampleDateList = Arrays.asList(sampleDates.split(","));
                        ...
                        //查询补点后样本曲线是否存在 []
                        //List<EventPowerSample> eventPowerSampleListEx = consCurveService.getCurveAmendByDate(sampleDateList , cons , baselinId);
                        //List<EventPowerSample> eventPowerSampleUpdateList = new ArrayList<>();
                        curveHisList = getPoint(curveHisList,consMethodMap2,consMethodMap3,startIndex,endIndex,consMethodMap5,new ArrayList<>());
                                    ...
                                    if(null!=eventPowerSamples && eventPowerSamples.size()>0) {
                                        ...
                                        if(null!=eventPowerSample) {
                                            for(int index = startIndex ; index <= endIndex ; index++){
                                                ...
                                                consMethodMap4.get(index).invoke(eventPowerSample,point);
                                ...
                                if ("1".equals(planBaseLine.getBaseLineCal())) {
                                    ...
                                    for (int i = startIndex; i <= endIndex; i++) {
                                        ...
                                        for (ConsCurve consCurve : consCurveList) {
                                            ...
                                            consCurveHis = (BigDecimal) consMethodMap2.get(i).invoke(consCurve);
                                    ...
                                    if (validSamplePowerList.size() > 0) {
                                        ...
                                        //负荷基线
                                        consCurveBase.setMaxLoadBaseline(maxSamplePower);
                                        consCurveBase.setMinLoadBaseline(minSamplePower);
                                        consCurveBase.setAvgLoadBaseline(avgSamplePower);
                                        consCurveBase.setNormal("Y");
                                        consCurveBase.setCalRule("1");

                                        //基线任务
                                        planBaseLine.setBaselineStatus("2");
                                        planBaseLine.setFailTimes(0);
                                        for (int i = startIndex; i <= endIndex; i++) {
                                            consMethodMap.get(i).invoke(consCurveBase,avgSamplePower);
                                        }
                                    ...
                                    if(null != existConsCurveBases && existConsCurveBases.size()>0) {
                                        ...
                                        //生成主键
                                        //String id =  HunanFinalCalculateStrategy.getId();
                                    ...
                                    //历史负荷
                                    consCurveList = curveHisList.stream().filter(con -> con.getConsId().equals(planBaseLine.getConsId()) &&
                                            sampleDateList.contains(simpleDateFormat.format(con.getDataDate()))).collect(Collectors.toList());
                                    if (null == consCurveList || consCurveList.size() == 0) {
                                        planBaseLine.setBaselineStatus("3");
                                        planBaseLine.setBaselineDesc("无历史负荷");
                                        if (null == planBaseLine.getFailTimes()) {
                        ...
                        String sql = "INSERT INTO dr_cons_baseline (\n" +
                                ...
                                "VALUES\n" +
                                "\t(? ,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                        String sql1 ="update dr_plan_baseline_task set BASELINE_STATUS=?,BASELINE_DESC=?,BASELINE_CAP_ID=?,FAIL_TIMES=?,UPDATE_TIME=? where id=?";

                        String sql2 = "INSERT INTO dr_event_power_sample (\n" +
                        ...
                        String sql5 = "UPDATE dr_event_power_sample \n" +
                        ...
                        String sql4 = "UPDATE dr_cons_baseline \n" +
                        ...
                        log.info("样本曲线保存成功,共"+eventPowerSampleList.size()+"条");
                        ...
                        log.info("样本曲线更新成功,共"+eventPowerSampleUpdateList.size()+"条");*/
                        //保存用户基线表
                        if(null!=insertList && insertList.size()>0) {
                            int k = 0;
                            conn2= DriverManager.getConnection(url,user,password);
                        ...
                        log.info("用户基线保存成功,共"+insertList.size()+"条");
                        //更新已存在的基线
                        if(null!=updateList && updateList.size()>0) {
                            int i = 0;
                        ...
                        log.info("用户基线更新成功,共" + updateList.size()+"条");
                        conn3= DriverManager.getConnection(url,user,password);
                        conn3.setAutoCommit(false);
                        ...
                        log.info("基线任务更新成功,共" + planBaseLines.size()+"条");
                        if(null!=conn1) {
                            conn1.commit();
                        }
                        if(null!=conn2) {
                        ...
                        if(null!=baseLine.getConsNum()) {
                            baseLine.setConsNum(baseLine.getConsNum()+1);
                            baseLineService.updateById(baseLine);
                        } else {
                            baseLine.setConsNum(1);
                            baseLineService.updateById(baseLine);
                        }
    ...
    public void synchroInvitationToPlan(String param) {
        log.info("用户方案开始同步!");
        // 查询当时所有事件
        EventParam eventParam = new EventParam();
        if(null!=param && !"".equals(param)) {
            if(StrategyUtils.isDate(param)) {
                eventParam.setRegulateDate(LocalDate.parse(param));
            } else {
                eventParam.setRegulateDate(LocalDate.now());
            }
        } else {
            eventParam.setRegulateDate(LocalDate.now());
        }
       ...
       /* statusList.add("02");*/
        statusList.add("03");
        statusList.add("06");
        eventParam.setStatusList(statusList);
        //只查询邀约事件
        eventParam.setTimeType("1");
        List<Event> todayEvents = (List<Event>) eventService.listToPlan(eventParam);
        if(null==todayEvents || todayEvents .size()==0) {
            log.info("无事件方案同步");
            return;
        }
        // 判断当前时间是否到了邀约日期的最大反馈截止时间
        if (CollectionUtil.isEmpty(todayEvents)) return;
        ExecutorService executor = ExecutorBuilder.create()
                .setCorePoolSize(corePoolSize)
                .setMaxPoolSize(maxPoolSize)
                .setWorkQueue(new LinkedBlockingQueue<>(workQueue))
                .build();
        executor.execute(this.synchroInvitationToPlan(todayEvents));
        executor.shutdown();
    }

    public Runnable synchroInvitationToPlan(List<Event> todayEvents) {
        Runnable runnable = new Runnable() {
            @Override
            public void run() {
                //查询事件截止时间集合(邀约表截止时间)
                List<Event> localDateTimes = consInvitationService.getMaxDeadlineTimeByCon(todayEvents);
                if (null == localDateTimes || localDateTimes.size() == 0) {
                    log.info("无事件信息");
                    return;
                }
                LocalDateTime deadlineTime = null;
                //查询事件参与所有用户
                List<ConsInvitation> consInvitations = consInvitationService.getConsInfoByEvents(todayEvents);
               /* if (null == consInvitations || consInvitations.size() == 0) {
                ...
                List<Long> planUp = new ArrayList<>();
                List<Long> eventIds = new ArrayList<>();
                for(Event event : todayEvents) {
                    eventIds.add(event.getEventId());
                }
                //查询所有用户方案
                List<PlanCons> planConsList = planConsService.getPlanNoDe(eventIds);
                ...
                wrapper.in(EventMonitorTask::getEventId, eventIds);
                List<EventMonitorTask> eventMonitorTaskList = eventMonitorTaskService.list(wrapper);
                //查询效果评估任务
                List<EvaluTask> evaluTaskList = evaluTaskService.getEvaluTaskByEvents(todayEvents);
                List<ConsInvitation> invitationsUpdateList = new ArrayList<>();
                List<PlanCons> planConsUpdateList = new ArrayList<>();
                List<Event> eventUpdatelist = new ArrayList<>();
                List<EvaluTask> evaluInsertTasks = new ArrayList<>();
                List<EvaluTask> evaluUpdateTasks = new ArrayList<>();
                for (Event todayEvent : todayEvents) {
                    String eventConsIds = "";
                            // 判断事件是否已经截至
                    List<Event> events = localDateTimes.stream().filter(time ->
                            time.getEventId().equals(todayEvent.getEventId())
                    ).collect(Collectors.toList());
                    if (null != events && events.size() > 0) {
                        deadlineTime = events.get(0).getDeadlineTime();
                    }
                    if (ObjectUtil.isNull(deadlineTime) || LocalDateTime.now().isBefore(deadlineTime)) {
                        // 没有最晚反馈截止时间，不进行同步
                        log.info(">>> 事件截止时间:{}", deadlineTime);
                        log.info(">>> 当前时间:{}", LocalDateTime.now());
                        continue;
                    }
                    //事件已截止,生成邀约截止短信
                   // sysSmsSendService.generateSms(String.valueOf(todayEvent.getEventId()), "15");
                    // 查询该事件的确认参与事件的用户，对所有的反馈用户进行排序
                    if(null==consInvitations) {
                        continue;
                    }
                    List<ConsInvitation> consInvitationList = consInvitations.stream().filter(consInvitation ->
                            consInvitation.getEventId().equals(todayEvent.getEventId())
                    ).collect(Collectors.toList());
                    if (CollectionUtil.isEmpty(consInvitationList)) continue;
                    // 根据排序后的顺序，将邀约信息同步更行到方案信息中
                    List<Plan> plans = planList.stream().filter(pla ->
                            pla.getRegulateId().equals(todayEvent.getEventId())
                    ).collect(Collectors.toList());
                    if (null == plans || plans.size() == 0) {
                        continue;
                    }
                    // 同步信息
                    for (int i = 0; i < consInvitationList.size(); i++) {
                        consInvitationList.get(i).setInvitationNo(i + 1);
                        consInvitationList.get(i).setSynchToPlan(YesOrNotEnum.Y.getCode());
                        invitationsUpdateList.add(consInvitationList.get(i));
                        for (PlanCons planCons : planConsList) {
                            if (consInvitationList.get(i).getConsId().equals(planCons.getConsId())
                            && plans.get(0).getPlanId().equals(planCons.getPlanId())) {
                                planCons.setReplyPrice(consInvitationList.get(i).getReplyPrice());
                                planCons.setDemandCap(consInvitationList.get(i).getReplyCap());
                                planCons.setSequenceNo(consInvitationList.get(i).getInvitationNo());
                                planCons.setInvolvedIn(consInvitationList.get(i).getIsParticipate());
                                planConsUpdateList.add(planCons);
                                eventConsIds = eventConsIds + planCons.getConsId() + "," ;
                            }
                        }
                    }
                    todayEvent.setSynchToPlan(YesOrNotEnum.Y.getCode());
                    todayEvent.setEventStatus(EventStatusEnum.STATUS_PASS.getCode());
                    eventUpdatelist.add(todayEvent);
                    planUp.add(plans.get(0).getPlanId());
                    // 同步数据到效果评估任务表
                    for (ConsInvitation consInvitation : consInvitationList) {
                        EvaluTask evaluTask = new EvaluTask();
                        evaluTask.setEventId(consInvitation.getEventId());
                        evaluTask.setConsId(consInvitation.getConsId());
                        evaluTask.setRegulateDate(todayEvent.getRegulateDate());
                        evaluTask.setStartTime(todayEvent.getStartTime());
                        evaluTask.setEndTime(todayEvent.getEndTime());
                        evaluTask.setIncentiveStandard(todayEvent.getIncentiveStandard());
                        evaluTask.setValidityJudgment(todayEvent.getValidityJudgment());
                        if (null != evaluTaskList && evaluTaskList.size() > 0) {
                            List<EvaluTask> evaluTasksData = evaluTaskList.stream().filter(evaluTask1 ->
                                    evaluTask1.getEventId().equals(todayEvent.getEventId())
                                    && evaluTask1.getConsId().equals(consInvitation.getConsId())
                            ).collect(Collectors.toList());
                            if (null != evaluTasksData && evaluTasksData.size() > 0) {
                                EvaluTask evaluConUpdateTask = evaluTasksData.get(0);
                                if (null != evaluConUpdateTask) {
                                    evaluTask.setId(evaluConUpdateTask.getId());
                                    evaluUpdateTasks.add(evaluTask);
                                } else {
                                    evaluInsertTasks.add(evaluTask);
                                }
                            } else {
                                evaluInsertTasks.add(evaluTask);
                            }
                    ...
                    //保存事件执行曲线任务表
                    List<EventMonitorTask> eventMonitorTasks = null;
                    EventMonitorTask eventMonitor = null;
                    if(null!=eventMonitorTaskList && eventMonitorTaskList.size()>0) {
                         eventMonitorTasks = eventMonitorTaskList.stream().filter(eventMonitorTask ->
                                eventMonitorTask.getEventId().equals(todayEvent.getEventId())
                        ).collect(Collectors.toList());
                    }
                    //将最后一个逗号去除
                    ...
                    //保存或更新事件曲线任务表
                    if(null!=eventMonitorTasks && eventMonitorTasks.size()>0) {
                        eventMonitor = eventMonitorTasks.get(0);
                        eventMonitor.setConsIds(eventConsIds);
                        eventMonitorTaskService.updateById(eventMonitor);
                    } else {
                        eventMonitor = new EventMonitorTask();
                        eventMonitor.setEventId(todayEvent.getEventId());
                        eventMonitor.setStartDate(todayEvent.getRegulateDate());
                        eventMonitor.setEndDate(todayEvent.getRegulateDate());
                        eventMonitor.setStartPeriod(todayEvent.getStartTime());
                        eventMonitor.setEndPeriod(todayEvent.getEndTime());
                        eventMonitor.setConsIds(eventConsIds);
                        eventMonitorTaskService.save(eventMonitor);
                    }
                }
                    PreparedStatement preparedStatement = null;
                    ...
                    try {
                        ...
                        Class.forName(driver);
                        //更新同步邀约信息状态
                        String sql = "UPDATE dr_cons_invitation set invitation_no=?,SYNCH_TO_PLAN=?,UPDATE_TIME=? where invitation_id=?";
                        if (null != invitationsUpdateList && invitationsUpdateList.size() > 0) {
                            //获取数据库连接
                            conn = DriverManager.getConnection(url, user, password);
                            conn.setAutoCommit(false);
                            preparedStatement = conn.prepareStatement(sql);
                            int i = 0;
                            for (ConsInvitation consInvitation : invitationsUpdateList) {
                                preparedStatement.setInt(1, consInvitation.getInvitationNo());
                                preparedStatement.setString(2, consInvitation.getSynchToPlan());
                                preparedStatement.setString(3, dateFormat.format(new Date()));
                                preparedStatement.setLong(4, consInvitation.getInvitationId());
                                preparedStatement.addBatch();
                                if ((i + 1) % 1000 == 0 || i == invitationsUpdateList.size() - 1) {
                                    //每1000条提交一次
                                    preparedStatement.executeBatch();
                                    //清空记录
                                    preparedStatement.clearBatch();
                        ...
                        log.info("更新同步邀约信息状态完成，共:" + invitationsUpdateList.size() + "条");
                        //更新方案信息
                        String sql2 = "update dr_plan_cons set reply_price=?,DEMAND_CAP=?,sequence_no=?,INVOLVED_IN=?,UPDATE_TIME=?,IMPLEMENT=? where partic_id=?";
                        if (null != planConsUpdateList && planConsUpdateList.size() > 0) {
                            conn2 = DriverManager.getConnection(url, user, password);
                            conn2.setAutoCommit(false);
                            preparedStatement2 = conn2.prepareStatement(sql2);
                            int i = 0;
                            for (PlanCons planCons : planConsUpdateList) {
                                preparedStatement2.setBigDecimal(1, planCons.getReplyPrice());
                                preparedStatement2.setBigDecimal(2, planCons.getDemandCap());
                                preparedStatement2.setInt(3, planCons.getSequenceNo());
                                preparedStatement2.setString(4, planCons.getInvolvedIn());
                                preparedStatement2.setString(5, dateFormat.format(new Date()));
                                preparedStatement2.setString(6, planCons.getInvolvedIn());
                                preparedStatement2.setLong(7, planCons.getParticId());
                                preparedStatement2.addBatch();
                                if ((i + 1) % 1000 == 0 || i == planConsUpdateList.size() - 1) {
                                    //每1000条提交一次
                                    preparedStatement2.executeBatch();
                                    //清空记录
                                    preparedStatement2.clearBatch();
                        ...
                        log.info("更新方案信息完成，共:" + planConsUpdateList.size() + "条");
                        //更新事件信息
                        String sql3 = "UPDATE dr_event set SYNCH_TO_PLAN=?,event_status=?,update_time=? where EVENT_ID=?";
                        if (null != eventUpdatelist && eventUpdatelist.size() > 0) {
                            //获取数据库连接
                            conn3 = DriverManager.getConnection(url, user, password);
                            conn3.setAutoCommit(false);
                            preparedStatement3 = conn3.prepareStatement(sql3);
                            ...
                            for (Event event : eventUpdatelist) {
                                preparedStatement3.setString(1, event.getSynchToPlan());
                                preparedStatement3.setString(2, event.getEventStatus());
                                preparedStatement3.setString(3, dateFormat.format(new Date()));
                                preparedStatement3.setLong(4, event.getEventId());
                                preparedStatement3.addBatch();
                                if ((i + 1) % 1000 == 0 || i == eventUpdatelist.size() - 1) {
                                    //每1000条提交一次
                                    preparedStatement3.executeBatch();
                                    //清空记录
                                    preparedStatement3.clearBatch();
                        ...
                        log.info("更新事件信息完成，共:" + eventUpdatelist.size() + "条");
                        String sql4 = "INSERT INTO dr_evalu_task (EVENT_ID,CONS_ID,REGULATE_DATE,START_TIME," +
                                "END_TIME,INCENTIVE_STANDARD,VALIDITY_JUDGMENT,CREATE_TIME,CREATE_USER,EVALU_TODAY_STATUS,EVALU_NEXTDAY_STATUS,SUBSIDY_STATUS,IMPLEMENT) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)";
                        if (evaluInsertTasks.size() > 0) {
                            //获取数据库连接
                            conn4 = DriverManager.getConnection(url, user, password);
                            conn4.setAutoCommit(false);
                            preparedStatement4 = conn4.prepareStatement(sql4);
                            int i = 0;
                            for (EvaluTask evaluTask : evaluInsertTasks) {
                                preparedStatement4.setLong(1, evaluTask.getEventId());
                                preparedStatement4.setString(2, evaluTask.getConsId());
                                preparedStatement4.setString(3, simpleDateFormat.format(evaluTask.getRegulateDate()));
                                preparedStatement4.setString(4, evaluTask.getStartTime());
                                preparedStatement4.setString(5, evaluTask.getEndTime());
                                preparedStatement4.setString(6, evaluTask.getIncentiveStandard());
                                preparedStatement4.setString(7, evaluTask.getValidityJudgment());
                                preparedStatement4.setString(8, dateFormat.format(new Date()));
                                preparedStatement4.setString(9, "-1");
                                preparedStatement4.setString(10, "1");
                                preparedStatement4.setString(11, "1");
                                preparedStatement4.setString(12, "1");
                                preparedStatement4.setString(13, "Y");
                                preparedStatement4.addBatch();
                                if ((i + 1) % 1000 == 0 || i == evaluInsertTasks.size() - 1) {
                                    //每1000条提交一次
                        ...
                        log.info("保存效果评估任务完成，共:" + evaluInsertTasks.size() + "条");
                        String sql5 = "UPDATE dr_evalu_task set EVENT_ID=?,CONS_ID=?,REGULATE_DATE=?,START_TIME=?,END_TIME=?,INCENTIVE_STANDARD=?,VALIDITY_JUDGMENT=?\n" +
                                "\t,CREATE_TIME=?,CREATE_USER=? where id=?";
                        if (evaluUpdateTasks.size() > 0) {
                            //获取数据库连接
                            conn5 = DriverManager.getConnection(url, user, password);
                            conn5.setAutoCommit(false);
                            preparedStatement5 = conn5.prepareStatement(sql5);
                            int i = 0;
                            for (EvaluTask evaluTask : evaluUpdateTasks) {
                                preparedStatement5.setLong(1, evaluTask.getEventId());
                                preparedStatement5.setString(2, evaluTask.getConsId());
                                preparedStatement5.setString(3, simpleDateFormat.format(evaluTask.getRegulateDate()));
                                preparedStatement5.setString(4, evaluTask.getStartTime());
                                preparedStatement5.setString(5, evaluTask.getEndTime());
                                preparedStatement5.setString(6, evaluTask.getIncentiveStandard());
                                preparedStatement5.setString(7, evaluTask.getValidityJudgment());
                                preparedStatement5.setString(8, dateFormat.format(new Date()));
                                preparedStatement5.setString(9, "-1");
                                preparedStatement5.setLong(10, evaluTask.getId());
                                preparedStatement5.addBatch();
                                if ((i + 1) % 1000 == 0 || i == evaluUpdateTasks.size() - 1) {
                                    //每1000条提交一次
                                    preparedStatement5.executeBatch();
                                    //清空记录
                                    preparedStatement5.clearBatch();
                        ...
                        log.info("更新效果评估任务完成，共:" + evaluUpdateTasks.size() + "条");
                        //保存信息到消息表
                       if(null!=planUp && planUp.size()>0) {
                           for(Long planId : planUp) {
                               Map<String, Object> map = new HashMap<>();
                               map.put("topic", resultTimeTopic);
                               map.put("message_key", planId);
    ...
    public void synchroCustInvitationToPlan(String param) {
        // 查询当时所有事件
        EventParam eventParam = new EventParam();
        if(null!=param && !"".equals(param)) {
            if(StrategyUtils.isDate(param)) {
        ...
        // eventParam.setSynchToPlan(YesOrNotEnum.N.getCode());
        List<String> statusList = new ArrayList<>();
        //待执行、执行中、已发布三种状态
       /* statusList.add("02");*/
        statusList.add("03");
        ...
        ExecutorService executor = ExecutorBuilder.create()
                ...
                .setWorkQueue(new LinkedBlockingQueue<>(workQueue))
                .build();
        executor.execute(this.synchroInvitationCustToPlan(todayEvents));
        executor.shutdown();
    }

    public Runnable synchroInvitationCustToPlan(List<Event> todayEvents) {
        Runnable runnable = new Runnable() {
            @Override
            public void run() {
                //查询事件截止时间集合
                List<Event> localDateTimes = custInvitationService.getMaxDeadlineTimeByCon(todayEvents);
                if (null == localDateTimes || localDateTimes.size() == 0) {
                    log.info("无事件信息");
                    return;
                }
                LocalDateTime deadlineTime = null;
                //查询事件参与所有客户
                List<CustInvitation> custInvitations = custInvitationService.getConsInfoByEvents(todayEvents);
                ...
                //查询所有用户方案
                List<PlanCust> planConsList1 = planCustService.getPlanNoDe(eventIds);
                if (null == planConsList1 || planConsList1.size() == 0) {
                    log.info("无用户信息");
                    return;
                }
                //查询效果评估任务
                List<EvaluCustTask> evaluTaskList = evaluCustTaskService.getEvaluTaskByEvents(todayEvents);
                List<CustInvitation> invitationsUpdateList = new ArrayList<>();
                List<PlanCust> planConsUpdateList = new ArrayList<>();
                //List<Event> eventUpdatelist = new ArrayList<>();
                List<EvaluCustTask> evaluInsertTasks = new ArrayList<>();
                List<EvaluCustTask> evaluUpdateTasks = new ArrayList<>();
                for (Event todayEvent : todayEvents) {
                    // 判断事件是否已经截至
                    List<Event> events = localDateTimes.stream().filter(time ->
                            time.getEventId().equals(todayEvent.getEventId())
                    ...
                    // 查询该事件的确认参与事件的用户，对所有的反馈用户进行排序
                    List<CustInvitation> custInvitationList = custInvitations.stream().filter(consInvitation ->
                            consInvitation.getEventId().equals(todayEvent.getEventId())
                    ).collect(Collectors.toList());
                    if (CollectionUtil.isEmpty(custInvitationList)) continue;
                    // 根据排序后的顺序，将邀约信息同步更行到方案信息中
                    List<Plan> plans = planList.stream().filter(pla ->
                            pla.getRegulateId().equals(todayEvent.getEventId())
                    ).collect(Collectors.toList());
                    ...
                    List<PlanCust> planCustList = planConsList1.stream().filter(planCons ->
                            planCons.getPlanId().equals(plans.get(0).getPlanId())
                    ).collect(Collectors.toList());
                    if (CollectionUtil.isEmpty(planCustList)) continue;
                    // 同步信息
                    for (int i = 0; i < custInvitationList.size(); i++) {
                        custInvitationList.get(i).setInvitationNo(i + 1);
                        custInvitationList.get(i).setSynchToPlan(YesOrNotEnum.Y.getCode());
                        invitationsUpdateList.add(custInvitationList.get(i));
                        for (PlanCust planCust : planCustList) {
                            if (custInvitationList.get(i).getCustId().equals(planCust.getCustId())
                                    && plans.get(0).getPlanId().equals(planCust.getPlanId())) {
                                planCust.setReplyPrice(custInvitationList.get(i).getReplyPrice());
                                planCust.setDemandCap(custInvitationList.get(i).getReplyCap());
                                planCust.setSequenceNo(custInvitationList.get(i).getInvitationNo());
                                planCust.setInvolvedIn(custInvitationList.get(i).getIsParticipate());
                                planConsUpdateList.add(planCust);
                            }
                        }
                    }
                    // 同步数据到效果评估任务表
                    for (CustInvitation consInvitation : custInvitationList) {
                        EvaluCustTask evaluTask = new EvaluCustTask();
                        evaluTask.setEventId(consInvitation.getEventId());
                        evaluTask.setCustId(consInvitation.getCustId());
                        evaluTask.setRegulateDate(todayEvent.getRegulateDate());
                        evaluTask.setStartTime(todayEvent.getStartTime());
                        evaluTask.setEndTime(todayEvent.getEndTime());
                        evaluTask.setIncentiveStandard(todayEvent.getIncentiveStandard());
                        evaluTask.setValidityJudgment(todayEvent.getValidityJudgment());
                        evaluTask.setIntegrator(consInvitation.getIntegrator());
                        if (null != evaluTaskList && evaluTaskList.size() > 0) {
                            List<EvaluCustTask> evaluTasksData = evaluTaskList.stream().filter(evaluTask1 ->
                                    evaluTask1.getEventId().equals(todayEvent.getEventId())
                                    && evaluTask1.getCustId().equals(consInvitation.getCustId())
                            ).collect(Collectors.toList());
                            if (null != evaluTasksData && evaluTasksData.size() > 0) {
                                EvaluCustTask evaluConUpdateTask = evaluTasksData.get(0);
                                if (null != evaluConUpdateTask) {
                                    evaluTask.setId(evaluConUpdateTask.getId());
                                    evaluUpdateTasks.add(evaluTask);
                                } else {
                    ...
                    try {
                        ...
                        Class.forName(driver);
                        //更新同步邀约信息状态
                        String sql = "UPDATE dr_cust_invitation set invitation_no=?,SYNCH_TO_PLAN=?,UPDATE_TIME=? where invitation_id=?";
                        if (null != invitationsUpdateList && invitationsUpdateList.size() > 0) {
                            //获取数据库连接
                            conn = DriverManager.getConnection(url, user, password);
                            conn.setAutoCommit(false);
                            preparedStatement = conn.prepareStatement(sql);
                            int i = 0;
                            for (CustInvitation custInvitation : invitationsUpdateList) {
                                preparedStatement.setInt(1, custInvitation.getInvitationNo());
                                preparedStatement.setString(2, custInvitation.getSynchToPlan());
                                preparedStatement.setString(3, dateFormat.format(new Date()));
                                preparedStatement.setLong(4, custInvitation.getInvitationId());
                                preparedStatement.addBatch();
                                if ((i + 1) % 1000 == 0 || i == invitationsUpdateList.size() - 1) {
                                    //每1000条提交一次
                                    preparedStatement.executeBatch();
                        ...
                        log.info("更新客户同步邀约信息状态完成，共:" + invitationsUpdateList.size() + "条");
                        //更新方案信息
                        String sql2 = "update dr_plan_cust set reply_price=?,DEMAND_CAP=?,sequence_no=?,INVOLVED_IN=?,UPDATE_TIME=?,IMPLEMENT=? where partic_id=?";
                        if (null != planConsUpdateList && planConsUpdateList.size() > 0) {
                            conn2 = DriverManager.getConnection(url, user, password);
                            conn2.setAutoCommit(false);
                            preparedStatement2 = conn2.prepareStatement(sql2);
                            int i = 0;
                            for (PlanCust planCust : planConsUpdateList) {
                                preparedStatement2.setBigDecimal(1, planCust.getReplyPrice());
                                preparedStatement2.setBigDecimal(2, planCust.getDemandCap());
                                preparedStatement2.setInt(3, planCust.getSequenceNo());
                                preparedStatement2.setString(4, planCust.getInvolvedIn());
                                preparedStatement2.setString(5, dateFormat.format(new Date()));
                                preparedStatement2.setString(6, planCust.getInvolvedIn());
                                preparedStatement2.setLong(7, planCust.getParticId());
                                preparedStatement2.addBatch();
                                if ((i + 1) % 1000 == 0 || i == planConsUpdateList.size() - 1) {
                                    //每1000条提交一次
                                    preparedStatement2.executeBatch();
                        ...
                        log.info("更新客户方案信息完成，共:" + planConsUpdateList.size() + "条");
                        String sql4 = "INSERT INTO dr_evalu_cust_task (EVENT_ID,CUST_ID,REGULATE_DATE,START_TIME," +
                                "END_TIME,INCENTIVE_STANDARD,VALIDITY_JUDGMENT,CREATE_TIME,CREATE_USER,integrator,EVALU_TODAY_STATUS,EVALU_NEXTDAY_STATUS,SUBSIDY_STATUS,IMPLEMENT) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                        if (evaluInsertTasks.size() > 0) {
                            //获取数据库连接
                            conn4 = DriverManager.getConnection(url, user, password);
                            conn4.setAutoCommit(false);
                            preparedStatement4 = conn4.prepareStatement(sql4);
                            int i = 0;
                            for (EvaluCustTask evaluTask : evaluInsertTasks) {
                                preparedStatement4.setLong(1, evaluTask.getEventId());
                                preparedStatement4.setLong(2, evaluTask.getCustId());
                                preparedStatement4.setString(3, simpleDateFormat.format(evaluTask.getRegulateDate()));
                                preparedStatement4.setString(4, evaluTask.getStartTime());
                                preparedStatement4.setString(5, evaluTask.getEndTime());
                                preparedStatement4.setString(6, evaluTask.getIncentiveStandard());
                                ...
                                preparedStatement4.setString(8, dateFormat.format(new Date()));
                                preparedStatement4.setString(9, "-1");
                                preparedStatement4.setString(10, evaluTask.getIntegrator());
                                preparedStatement4.setString(11, "1");
                                preparedStatement4.setString(12, "1");
                                preparedStatement4.setString(13, "1");
                                preparedStatement4.setString(14, "Y");
                                preparedStatement4.addBatch();
                                if ((i + 1) % 1000 == 0 || i == evaluInsertTasks.size() - 1) {
                                    //每1000条提交一次
                                    preparedStatement4.executeBatch();
                        ...
                        log.info("保存效果评估任务完成，共:" + evaluInsertTasks.size() + "条");
                        String sql5 = "UPDATE dr_evalu_cust_task set EVENT_ID=?,CUST_ID=?,REGULATE_DATE=?,START_TIME=?,END_TIME=?,INCENTIVE_STANDARD=?,VALIDITY_JUDGMENT=?\n" +
                                "\t,CREATE_TIME=?,CREATE_USER=?,integrator=? where id=?";
                        if (evaluUpdateTasks.size() > 0) {
                            //获取数据库连接
                            conn5 = DriverManager.getConnection(url, user, password);
                            conn5.setAutoCommit(false);
                            preparedStatement5 = conn5.prepareStatement(sql5);
                            int i = 0;
                            for (EvaluCustTask evaluTask : evaluUpdateTasks) {
                                preparedStatement5.setLong(1, evaluTask.getEventId());
                                preparedStatement5.setLong(2, evaluTask.getCustId());
                                preparedStatement5.setString(3, simpleDateFormat.format(evaluTask.getRegulateDate()));
                                preparedStatement5.setString(4, evaluTask.getStartTime());
                                preparedStatement5.setString(5, evaluTask.getEndTime());
                                preparedStatement5.setString(6, evaluTask.getIncentiveStandard());
                                ...
                                preparedStatement5.setString(8, dateFormat.format(new Date()));
                                preparedStatement5.setString(9, "-1");
                                preparedStatement5.setString(10, evaluTask.getIntegrator());
                                preparedStatement5.setLong(11, evaluTask.getId());
                                preparedStatement5.addBatch();
                                if ((i + 1) % 1000 == 0 || i == evaluUpdateTasks.size() - 1) {
                                    //每1000条提交一次
                                    preparedStatement5.executeBatch();
                        ...
                        log.info("更新客户效果评估任务完成，共:" + evaluUpdateTasks.size() + "条");
                        if(null!=conn) {
                            conn.commit();
                        }
                        if(null!=conn2) {
