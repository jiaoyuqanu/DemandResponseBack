...
//    @BusinessLog(title = "执行方案用户分页查询", opType = LogAnnotionOpTypeEnum.QUERY)
    ...
    /**
     * @description: 执行监测-用户监测-导出
     * @param:
     * @return:
     * @author: PengChuqing
     * @date: 2021/10/27 9:46
     */
    // @BusinessLog(title = "执行监测-用户监测-导出", opType = LogAnnotionOpTypeEnum.QUERY)
    @ApiOperation(value = "执行监测-用户监测-导出", notes = "执行监测-用户监测-导出", produces = "application/json")
    @PostMapping("/exportConsMonitor")
    public void exportConsMonitor(@RequestBody PlanConsParam planConsParam,HttpServletResponse response, HttpServletRequest request) throws ExecutionException, InterruptedException{
        if(null==planConsParam.getOrgId() || "".equals(planConsParam.getOrgId())) {
            planConsParam.setOrgId("34101");
        }
        Map<String,Object> dataMap = planConsService.exportConsMonitor(planConsParam);
        List<PlanConsMonitorExportVO> list = (List<PlanConsMonitorExportVO>) dataMap.get("baseLineAll");
        List<PlanConsMonitorExportVO> list3 = (List<PlanConsMonitorExportVO>) dataMap.get("curveAll");
        List<PlanConsMonitorExportVO> list4 = (List<PlanConsMonitorExportVO>) dataMap.get("responseAll");
        List<PlanConsMonitorExportVO> list5 = (List<PlanConsMonitorExportVO>) dataMap.get("executeRateAll");
        Workbook workBook = null;
        String fileName = null;
        try {
            //设置文件名称
            fileName = URLEncoder.encode("执行监测用户监测导出", "UTF-8");
            //设置文件类型
            response.setContentType("application/vnd.ms-excel");
            //设置编码格式
            response.setCharacterEncoding("utf-8");
            ...
            String titleName = event.getEventName() + "用户执行监测信息";
            List<Map<String, Object>> sheetList = new ArrayList<>();

            ExportParams consCurveModelExportParams2 = new ExportParams();
            consCurveModelExportParams2.setTitle(titleName);
            consCurveModelExportParams2.setSheetName("执行方案用户全天基线");
            // 创建sheet1使用得map
            Map<String, Object> consCurveModelMap = new HashMap<>();
            // title的参数为ExportParams类型，目前仅仅在ExportParams中设置了sheetName
            consCurveModelMap.put("title", consCurveModelExportParams2);
            // 模版导出对应得实体类型
            consCurveModelMap.put("entity", PlanConsMonitorExportVO.class);
            // sheet中要填充得数据
            consCurveModelMap.put("data", list);

            ExportParams consCurveModelExportParams3 = new ExportParams();
            consCurveModelExportParams3.setTitle(titleName);
            consCurveModelExportParams3.setSheetName("执行方案用户执行曲线");
            // 创建sheet1使用得map
            Map<String, Object> consCurveModelMap2 = new HashMap<>();
            // title的参数为ExportParams类型，目前仅仅在ExportParams中设置了sheetName
            consCurveModelMap2.put("title", consCurveModelExportParams3);
            // 模版导出对应得实体类型
            consCurveModelMap2.put("entity", PlanConsMonitorExportVO.class);
            // sheet中要填充得数据
            consCurveModelMap2.put("data", list3);

            ExportParams consCurveModelExportParams4 = new ExportParams();
            consCurveModelExportParams4.setSheetName("执行方案用户压降负荷");
            consCurveModelExportParams4.setTitle(titleName);
            // 创建sheet1使用得map
            Map<String, Object> consCurveModelMap3 = new HashMap<>();
            // title的参数为ExportParams类型，目前仅仅在ExportParams中设置了sheetName
            consCurveModelMap3.put("title", consCurveModelExportParams4);
            // 模版导出对应得实体类型
            consCurveModelMap3.put("entity", PlanConsMonitorExportVO.class);
            // sheet中要填充得数据
            consCurveModelMap3.put("data", list4);

            ExportParams consCurveModelExportParams5 = new ExportParams();
            consCurveModelExportParams5.setTitle(titleName);
            consCurveModelExportParams5.setSheetName("执行方案用户执行率");
            // 创建sheet1使用得map
            Map<String, Object> consCurveModelMap4 = new HashMap<>();
            // title的参数为ExportParams类型，目前仅仅在ExportParams中设置了sheetName
            consCurveModelMap4.put("title", consCurveModelExportParams5);
            // 模版导出对应得实体类型
            consCurveModelMap4.put("entity", PlanConsMonitorExportVO.class);
            // sheet中要填充得数据
            consCurveModelMap4.put("data", list5);
            sheetList.add(consCurveModelMap);
            sheetList.add(consCurveModelMap2);
            sheetList.add(consCurveModelMap3);
            sheetList.add(consCurveModelMap4);
    ...
    public void exportUserCustLine(@RequestBody EventParam eventParam,HttpServletResponse response, HttpServletRequest request) {
        ...
        Cons cons = consService.getById(eventParam.getConsId());
        if(cons == null){
            throw  new ServiceException(500,"该电力户号未找到用户");
        }
            ...
            if(!StringUtils.isEmpty(simplesId)){
                ...
                if(!CollectionUtils.isEmpty(eventPowerSampleAlls)){
                    for (EventPowerSampleAll eventPowerSampleAll : eventPowerSampleAlls) {
                        ...
                        eventPowerSampleAllModel.setConsName(cons.getConsName());
            ...
            if(!StringUtils.isEmpty(simplesId)){
                ...
                if(!CollectionUtils.isEmpty(eventPowerSamples)){
                    for (EventPowerSample eventPowerSample : eventPowerSamples) {
                        ...
                        eventPowerSampleModel.setConsName(cons.getConsName());
            ...
            if(LocalDate.now().equals(event.getRegulateDate())){
                ...
                if(!CollectionUtils.isEmpty(consCurveTodays)){
                    for (ConsCurveToday consCurveToday : consCurveTodays) {
                        ...
                        consCurveModel.setConsName(cons.getConsName());
