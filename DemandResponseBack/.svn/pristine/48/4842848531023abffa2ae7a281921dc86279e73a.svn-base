...
public class ConsEvaluationImmediateServiceImpl extends ServiceImpl<ConsEvaluationImmediateMapper, ConsEvaluationImmediate> implements ConsEvaluationImmediateService {
    ...
    public void exportEvaluationImmediate(EvaluationImmediateParam evaluationImmediateParam) {
        ...
        String[] titleRowCons = {"电力营销户号", "用户名称", "供电单位", "市供电单位", "县供电单位", "认约负荷(kW)", "是否有效响应", "实际响应负荷(kW)", "核定响应负荷(kW)", "实际最大负荷(kW)", "实际平均负荷(kW)", "最大基线负荷(kW)", "平均基线负荷(kW)"};
...
//        }
        ...
        for (int i = 0; i < evaluationCons.size(); i++) {
            ...
            String place = "";
            try {
                if (ObjectUtil.isNotEmpty(evaluationImmediate.getOrgNo())) {
                    Map<String, Object> cityAndCountyMap = cityAndCountyUtils.cityAndCounty(evaluationImmediate.getOrgNo(), jsonObject);
           ...
           /* dataRowsCons[i][3] = evaluationImmediate.getCityName();
            ...
            JSONArray jsonArray = data.stream().filter(item -> ((Map<String, Object>) item).get("id").equals(evaluationImmediate.getOrgNo())).collect(Collectors.toCollection(JSONArray::new));
            if (ObjectUtil.isNotEmpty(jsonArray) && jsonArray.size() > 0) {
                Map<String, Object> o = (Map<String, Object>) jsonArray.get(0);
                dataRowsCons[i][2] = (String) o.get("name");
            }
            dataRowsCons[i][3] = city;
            dataRowsCons[i][4] = county;
            dataRowsCons[i][5] = String.valueOf(Optional.ofNullable(evaluationImmediate.getReplyCap()).orElse(BigDecimal.ZERO));
            dataRowsCons[i][6] = effectiveName;
            dataRowsCons[i][7] = String.valueOf(Optional.ofNullable(evaluationImmediate.getActualCap()).orElse(BigDecimal.ZERO));
            dataRowsCons[i][8] = String.valueOf(Optional.ofNullable(evaluationImmediate.getConfirmCap()).orElse(BigDecimal.ZERO));
            dataRowsCons[i][9] = String.valueOf(Optional.ofNullable(evaluationImmediate.getMaxLoadActual()).orElse(BigDecimal.ZERO));
            dataRowsCons[i][10] = String.valueOf(Optional.ofNullable(evaluationImmediate.getAvgLoadActual()).orElse(BigDecimal.ZERO));
            dataRowsCons[i][11] = String.valueOf(Optional.ofNullable(evaluationImmediate.getMaxLoadBaseline()).orElse(BigDecimal.ZERO));
            dataRowsCons[i][12] = String.valueOf(Optional.ofNullable(evaluationImmediate.getAvgLoadBaseline()).orElse(BigDecimal.ZERO));
