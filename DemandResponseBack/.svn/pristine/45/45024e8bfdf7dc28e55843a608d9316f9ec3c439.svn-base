...
//    @BusinessLog(title = "需求响应项目修改状态", opType = LogAnnotionOpTypeEnum.EDIT)
    ...
    @BusinessLog(title = "基线申诉", opType = LogAnnotionOpTypeEnum.QUERY)
    @ApiOperation(value = "基线申诉", notes = "基线申诉", produces = "application/json")
    @PostMapping("/getBaselineGrievance")
    public ResponseData getBaselineGrievance(@RequestBody EventParam eventParam) {
        return ResponseData.success(projectService.getBaselineGrievance(eventParam));
    }

    @BusinessLog(title = "当前基线负荷数据", opType = LogAnnotionOpTypeEnum.QUERY)
    @ApiOperation(value = "当前基线负荷数据", notes = "当前基线负荷数据", produces = "application/json")
    @PostMapping("/getBaselineGrievanceData")
    public ResponseData getBaselineGrievanceData(@RequestBody EventParam eventParam) {
        Page<BaseLineDetail> baselineGrievanceData = new Page<>();
        try {
            baselineGrievanceData = projectService.getBaselineGrievanceData(eventParam);
        } catch (Exception e) {
            return ResponseData.fail("500",e.getMessage(),e.getMessage());
        }
        return ResponseData.success(baselineGrievanceData);
    }

    @BusinessLog(title = "时间段基线负荷估算", opType = LogAnnotionOpTypeEnum.QUERY)
    @ApiOperation(value = "时间段基线负荷估算", notes = "时间段基线负荷估算", produces = "application/json")
    @PostMapping("/getBaselineGrievanceCalculate")
    public ResponseData getBaselineGrievanceCalculate(@RequestBody ConsBaselineAllParam consBaselineAllParam) {
        return ResponseData.success(projectService.getBaselineGrievanceCalculate(consBaselineAllParam));
    }
