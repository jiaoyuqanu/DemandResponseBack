...
import com.xqxy.dr.modular.powerplant.mapper.DrDaVoidancePowerMapper;
 ...
 */
@Service
public class DrDaVoidancePowerServiceImpl extends ServiceImpl<DrDaVoidancePowerMapper, DrDaVoidancePower> implements DrDaVoidancePowerService {

    @Override
    public Map<String, Object> getVoidancePower(DrDaVoidanceParam param) {
        LambdaQueryWrapper<DrDaVoidancePower> queryWrapper = new LambdaQueryWrapper<>();
        if (ObjectUtil.isNotEmpty(param.getId())) {
            queryWrapper.eq(DrDaVoidancePower::getVoidanceId, param.getId());
        }
        DrDaVoidancePower baseLineDetailData = null;
        List<DrDaVoidancePower> baseLineDetailDatas = this.list(queryWrapper);
