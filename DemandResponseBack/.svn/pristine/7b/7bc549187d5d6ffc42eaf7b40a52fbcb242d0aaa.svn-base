...
public class AnHuiLocalDataAccessStrategy implements DataAccessStrategy {
    ...
    @Value("${anhuiConsUrl:http://725b83c34ece4985bab67cab7df16df9.apigateway.res.sgmc.sgcc.com.cn/cst/zhny/rds/post_zhnyQueryConsData_htlx}")
    public String anhuiConsUrl;

    @Value("${anhuiConsAppCode:0feb09f4a16c47b79ff39d0a79ec813b}")
    public String anhuiConsAppCode;
    ...
    class RequestField {
        ...
        public String toString() {
            return "RequestField{" + "fieldName='" + fieldName + '\'' + ", fieldValue='" + fieldValue + '\'' + ", operator='" + operator + '\'' + '}';
        }
    }
...
//        consCurve.setConsNo(cons.getElecConsNo());
    ...
    public Cons getConsFromMarketing(String elecNo, String consName, String orgNo) {
        if (ObjectUtil.isNull(elecNo)) {
            return null;
        }
        String response = AnhuiAdsCstUtils.queryCons(anhuiConsUrl, anhuiConsAppCode, 1, 10, elecNo);
        ...
        String dataString = jsonObject.getStr("data");
        if (dataString == null) {
            return null;
        }

        JSONObject dataObject = JSONUtil.parseObj(dataString);
        Integer consNum = dataObject.getInt("totalNum");
        if (consNum == null || consNum == 0) {
            return null;
        }

        cn.hutool.json.JSONArray consList = dataObject.getJSONArray("rows");
        if (consList.isEmpty()) {
            return null;
        }

        JSONObject objectObjectMap = consList.getJSONObject(0);
        ...
        if (ObjectUtil.isNotNull(objectObjectMap.get("CONTRACTCAP"))) {
            resCons.setContractCap(BigDecimal.valueOf(Long.parseLong(String.valueOf(objectObjectMap.get("CONTRACTCAP")))));
            ;//合同-容量
        }
        if (ObjectUtil.isNotNull(objectObjectMap.get("SECURITYCAP"))) {
            resCons.setSafetyLoad(BigDecimal.valueOf(Long.parseLong(String.valueOf(objectObjectMap.get("SECURITYCAP")))));
            ;//保安容量
        }
        ...
        if (ObjectUtil.isNotNull(objectObjectMap.get("RUNCAP"))) {
            resCons.setRunCap(BigDecimal.valueOf(Long.parseLong(String.valueOf(objectObjectMap.get("RUNCAP")))));
            ;//运行容量
        }
        ...
        if (ObjectUtil.isNotNull(objectObjectMap.get("ORGNO"))) {
            resCons.setOrgNo(String.valueOf(objectObjectMap.get("ORGNO")));
            ;//供电单位编码
        }
        if (ObjectUtil.isNotNull(objectObjectMap.get("CONSID"))) {
            resCons.setCustId(Long.valueOf(String.valueOf(objectObjectMap.get("CONSID"))));
            ;//唯一主键
        }
